...
rocstar_dir = '/home/khoffmey/work/PubBench/rocstar_metrics'
nvidia_dir = '/home/khoffmey/work/PubBench/nvidia_metrics'
...
for subdir in glob.glob(os.path.join(rocstar_dir, '*/')):
    gpu = subdir.split('/')[-2]
...
for subdir in glob.glob(os.path.join(nvidia_dir, '*/')):
    gpu = subdir.split('/')[-2]
    for file in glob.glob(os.path.join(subdir, '*')):
        filename = file.split('/')[-1]
        if len(filename.split('_')) == 5:
            ...
            if not ai_col:
                print('ai not found!')
                exit(1)
            power_df['power.draw [W]'] = power_df['power.draw [W]'].apply(lambda x: round(float(str(x)[1:-2])))
