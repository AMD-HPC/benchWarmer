// Copyright (C) 2025 Advanced Micro Devices, Inc.
// SPDX-License-Identifier: MIT

#include "benchWarmer_run_operations.cuh"
#include "benchWarmer_api.h"

void run_fp64_benchmarks(
		bool add, bool mul, bool muladd, bool div, bool rsq, bool mfma,
		int grid_size, int block_size, int buffer_size, int experiments,
		GPUArch arch)
{
#if __HIPCC__
	if (arch == GPUArch::GFX90A || arch == GPUArch::GFX940 || arch == GPUArch::GFX950) {
		run_operations<double>(add, mul, muladd, div, rsq, false, mfma,
				grid_size, block_size, buffer_size, experiments, arch);
	} else {
		run_operations_not_mfma<double>(add, mul, muladd, div, rsq, false,
				grid_size, block_size, buffer_size, experiments, arch);
	}
#elif __CUDACC__
	run_operations<double>(add, mul, muladd, div, rsq, false, mfma,
			grid_size, block_size, buffer_size, experiments, arch);
#endif
}
